...
from game import Game, GameFrame, generate_rules
...
def main():
    ...
    num_options = int(input("How many options will this game have? "))
    rules = generate_rules(num_options)
    rps_options = list(rules.keys())
    max_wins = int(input("How many wins are needed to win the set? "))
    ...
    print(f'The score is {player1_wins}-{player2_wins}')
